...
export default function DriverDashboard() {
  ...
  // 1. 현재 기사에게 배정된 병원 목록 조회 (assignedDriverId 기반)
  ...
  if (isUserLoading || isHospLoading) return (
    <div className="p-12 text-center text-slate-400 font-bold bg-slate-900 min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-200">운송 정보를 불러오는 중입니다...</p>
    </div>
  );
      ...
      <section className="flex justify-between items-end py-4">
        <div className="space-y-1">
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">
            담당 병원: <span className="text-secondary">{assignedHospitals?.length || 0}개 거점</span>
          </p>
        </div>
        <Button variant="ghost" size="sm" asChild className="text-secondary font-bold text-xs">
          <Link href="/driver/hospitals">내 병원 관리 <ChevronRight className="h-3 w-3" /></Link>
        </Button>
      </section>

      {assignedHospitalIds.length === 0 ? (
        <div className="p-12 text-center bg-slate-800/50 rounded-3xl border-2 border-dashed border-white/5">
          <Truck className="h-12 w-12 text-slate-700 mx-auto mb-4" />
          <p className="text-slate-400 font-bold">배정된 병원이 없습니다.</p>
          <p className="text-[11px] text-slate-500 mt-2">관리자가 시스템에서 기사님을 특정 병원에 배정해야 합니다.</p>
        </div>
      ) : (
        <>
          <div className="space-y-4">
            ...
            {isReqLoading ? (
              <div className="p-12 text-center text-slate-500">데이터 로드 중...</div>
                      ...
                      <Link href={`/driver/collection/${req.id}`}>
                        <div className="bg-white/5 p-6 border-t border-white/5 flex items-center justify-between hover:bg-white/10 transition-all group">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-sm font-black text-white">현장 수거 대조 시작</p>
                              <p className="text-xs text-slate-400 font-medium">수량 확인 및 증빙 등록</p>
                            </div>
                          </div>
              ...
              <div className="p-12 text-center text-slate-500 bg-slate-800/30 rounded-3xl border border-dashed border-white/5">
                현재 대기 중인 수거 요청이 없습니다.
              </div>
            )}
          </div>

          <section className="space-y-4">
            <h2 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] px-2">납품 및 출고 대기 ({deliveryList.length})</h2>
                    ...
                    <CardContent className="p-5 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-indigo-400 font-bold bg-indigo-500/10 px-2 py-0.5 rounded-full inline-block mt-1">납품 대기 중</p>
                        </div>
                      </div>

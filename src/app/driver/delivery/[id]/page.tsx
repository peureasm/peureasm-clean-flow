...
import { ChevronLeft, Truck, Package, Info, CheckCircle2 } from 'lucide-react';
...
export default function DriverDeliveryDetail() {
  ...
  if (isReqLoading || isItemsLoading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 gap-4">
      ...
      <p className="text-slate-400 font-bold">배송 데이터 로드 중...</p>
    </div>
  );

  if (!request) return <div className="p-8 text-center text-white bg-slate-900 min-h-screen">데이터를 찾을 수 없습니다.</div>;
  ...
  return (
    <div className="bg-slate-900 min-h-screen pb-32 text-slate-50">
      <div className="sticky top-0 z-20 bg-slate-900/90 backdrop-blur-md border-b border-white/10 p-4 flex items-center justify-between">
        ...
        <h1 className="text-lg font-black tracking-tight text-white">최종 납품 확인</h1>
        <div className="w-10"></div>
      </div>

      <div className="p-4 space-y-6">
        <section className="bg-indigo-600/10 rounded-3xl p-6 border border-indigo-500/30 shadow-2xl relative overflow-hidden">
        ...
        <section className="space-y-4">
          ...
          {items.map((item) => {
            ...
            return (
              <Card key={item.id} className="bg-slate-800 border-none rounded-3xl overflow-hidden ring-1 ring-white/10">
                <CardContent className="p-6 space-y-4">
                  <div className="flex justify-between items-center">
                    <div className="space-y-1">
                      <p className="font-black text-xl text-white">{itemName}</p>
                      <div className="flex flex-wrap gap-2">
                        <Badge variant="secondary" className="bg-slate-900 text-slate-400 border-none text-[10px]">수거: {item.verifiedQuantity || 0}</Badge>
                        <Badge variant="secondary" className="bg-indigo-900/30 text-indigo-400 border-none text-[10px]">입고: {item.inboundQuantity || item.verifiedQuantity || 0}</Badge>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <Input 
                        ...
                        className="w-24 bg-slate-900 border-white/10 text-right font-black text-2xl h-14 rounded-2xl text-indigo-400 focus:ring-indigo-500" 
                      />
                    </div>
                  </div>
                </CardContent>
        ...
        <section className="bg-slate-800/50 rounded-3xl p-6 space-y-4 border border-white/5">
          <div className="flex items-center gap-3">
            ...
            <p className="text-base font-black text-white">병원 담당자 확인</p>
          </div>
          <p className="text-sm text-slate-300 leading-relaxed font-medium">납품 시 병원 담당자와 수량을 대조하고 최종 확인을 받았음을 서약합니다.</p>
        </section>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-slate-900/90 backdrop-blur-xl border-t border-white/10 flex gap-3 z-30 max-w-lg mx-auto">
        <Button 
          className="w-full h-16 rounded-2xl font-black text-lg gap-2 bg-indigo-600 text-white shadow-2xl shadow-indigo-500/20 hover:bg-indigo-500 transition-all"
